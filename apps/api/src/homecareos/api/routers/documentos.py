"""`GET /api/documentos`, `GET /api/documentos/{id}` e o stub de revalidação.

`POST /api/documentos` **não** mora aqui — continua em
`homecareos.intake.router`, que esta trilha não toca (é o contrato já
consumido pelo frontend). Este router só acrescenta leitura e o stub de
revalidação; nenhum dos dois endpoints daqui escreve em `documentos`.
"""
...
@router.post(
    ...
    summary="Revalida um documento contra as regras da operadora (não implementado)",
    description=(
        "Stub honesto: o motor de regras (issue #5) está sendo escrito em outra trilha, "
        "em paralelo a esta. Sempre responde 501 — nunca simula uma revalidação."
    ),
)
def revalidar_documento(documento_id: uuid.UUID) -> None:
    raise HTTPException(
        status_code=status.HTTP_501_NOT_IMPLEMENTED,
        detail=(
            "revalidação ainda não disponível: o motor de regras (issue #5) está em "
            "desenvolvimento em outra trilha"
        ),
    )
