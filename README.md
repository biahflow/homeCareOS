...
O `--profile tools` importa: `api`, `api-migrate`, `api-seed`, `api-alertas`,
`api-email-teste` e `api-retencao` compartilham o mesmo Dockerfile mas
constroem imagens **separadas**, e um `build` sem ele alcança só a do `api`. O
comando acima reconstrói também o `web`, que não está em profile nenhum.
